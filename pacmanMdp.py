...
class PacmanMdp(mdp.MarkovDecisionProcess):
    ...
    def updateTransitionFunction(self, stateL, action, nextStateL):    
      ...
      #  util.raiseNotDefined()
        ...
        #Si el par estado accion no se encuentra en el diccionario se anade junto al contador
        if  not (state, action) in  self.frecuencias.keys():
            self.frecuencias[(state, action)] = util.Counter()

		#Cada vez que aparezca un par estado accion y el mismo estado siguiente se suma 1 al contador	
        self.frecuencias[(state, action)][nextState] += 1        
    ...
    def getTransitionStatesAndProbabilities(self, state, action):
       ...
       # util.raiseNotDefined()
		...
		#Mediante el metodo totalCount hallamos los casos totales
        if (state, action) in self.frecuencias.keys():
            nCasosTotales = self.frecuencias[(state, action)].totalCount()
            
			#En cada estadoSiguiente de un par estado-accion hallamos la probabilidad
			#Casos Favorables/totales
			#Devolvemos el estado siguiente y su probabilidad
            for nextState in self.frecuencias[(state,action)].keys():
                prob = self.frecuencias[(state,action)][nextState]/nCasosTotales
